  ...
  <div class="login_container">
    <el-row>
      ...
      <el-col :span="12" :xs="24">
        <el-form ref="ruleFormRef" class="login_form" :rules="formRules" :model="loginData" :scroll-to-error="true"
          :inline-message="true" :show-message="true">
          <h1>{{ $t('login.title') }}</h1>
          <h2>{{ $t('login.subtitle') }}</h2>
          <!-- 用户名 -->
          <el-form-item prop="username">
            <el-input v-model="loginData.username" :prefix-icon="User" clearable autocomplete="on" autofocus
          ...
          <el-form-item prop="password">
            ...
            <el-input v-model="loginData.password" :prefix-icon="Lock" show-password maxlength="16" minlength="4"
              clearable :placeholder="$t('login.passwordPlaceholder')"></el-input>
          </el-form-item>
          <!-- 登录按钮 -->
          <div class="login_btn_container">

            <el-button class="btn login_btn" type="primary" :loading="false" @click="submitForm(ruleFormRef)">{{
              $t('login.login') }}</el-button>

            <el-button class="btn forget_pwd_btn" type="primary" :loading="false" @click="goForgetPassword">{{
